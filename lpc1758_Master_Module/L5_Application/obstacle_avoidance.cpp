...
ride_states gps_drive_mode(GPS_CAR_RUNNING_t gps_car_running)
{
	...
	switch(gps_car_running.GPS_CAR_ANGLE_TURN_dirmsg)
	{
		...
		case 7: car_should = stop; break;
		case 8: car_should = stop; break;
...
void* set_motor_direction_and_speed(Indicator *car, range_type left, range_type center, range_type right, range_type rear, void* temp_gps_running)
{
	static MOTOR_SIGNAL_t set_motor= {0};
	ride_states car_state_is = stop;

	GPS_CAR_RUNNING_t gps_car_running;
	memcpy(&gps_car_running, temp_gps_running, sizeof(set_motor));
	// Resetting Structure Members
	memset(&set_motor, 0, sizeof(set_motor));
	if(gps_car_running.GPS_CAR_ANGLE_TURN_dirmsg == 7 || gps_car_running.GPS_CAR_DISTANCE_to_destination == 8)
	{
